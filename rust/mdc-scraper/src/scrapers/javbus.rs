//! JAVBus scraper
//!
//! JAVBus (javbus.com) is a popular JAV metadata database with extensive coverage.
//! It provides movie information, covers, actors, and links to various resources.
...
/// JAVBus scraper implementation
pub struct JavbusScraper {
    /// Base URL (can be different mirrors: javbus.com, javsee.com, etc.)
    pub base_url: String,
}

impl JavbusScraper {
    /// Create a new JAVBus scraper with default URL
    pub fn new() -> Self {
        Self {
            base_url: "https://www.javbus.com".to_string(),
        }
    }
    ...
    fn extract_info_value(&self, html: &Html, label: &str) -> String {
        // JAVBus uses structure like: <p><span class="header">Label:</span> Value</p>
        if let Ok(selector) = Selector::parse("div.info p") {
...
impl Default for JavbusScraper {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Scraper for JavbusScraper {
    ...
    async fn query_number_url(&self, number: &str) -> Result<String> {
        // JAVBus URL pattern: /ABC-001
        let clean_number = number.to_uppercase();
        Ok(format!("{}/{}", self.base_url, clean_number))
    }
    ...
    fn parse_metadata(&self, html: &Html, url: &str) -> Result<MovieMetadata> {
        ...
        metadata.title = self.select_text(html, "div.container h3, h3");
        ...
        let cover = self.select_attr(
            html,
            "a.bigImage img, img.bigImage, div.screencap img",
            "src",
        );
        if !cover.is_empty() {
        ...
        metadata.actor = self.select_all_text(
            html,
            "div.star-name a, div#avatar-waterfall a .photo-info span, .avatar-box span",
        );
        metadata.actor.retain(|a| !a.is_empty() && a.len() > 1);

        // Genres/Tags: <span class="genre"> <a> or <label>
        metadata.tag = self.select_all_text(html, "span.genre a, p.header a.genre");
        metadata.tag.retain(|t| !t.is_empty());

        // Rating: JAVBus might show ratings differently
        // Look for star ratings or numeric scores
        let rating_container = self.select_text(html, "span.score, div.rating");
        ...
        // Outline: Usually constructed from genres on JAVBus
        ...
        if metadata.title.is_empty() {
            return Err(anyhow!("Failed to extract title from JAVBus"));
        }
        if metadata.cover.is_empty() {
            return Err(anyhow!("Failed to extract cover image from JAVBus"));
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_javbus_scraper_url() {
        let scraper = JavbusScraper::new();
        let url = scraper.query_number_url("ABP-001").await.unwrap();
        assert!(url.contains("javbus.com"));
        assert!(url.contains("ABP-001"));
    }

    #[test]
    fn test_javbus_scraper_parse() {
        let scraper = JavbusScraper::new();

        let html_content = r#"
        <html>
        <head>
            <title>ABP-001 - JAVBus</title>
        </head>
        <body>
            <div class="container">
                <h3>ABP-001 Test JAV Movie</h3>
                <a class="bigImage">
        ...
        let metadata = scraper
            .parse_metadata(&html, "https://www.javbus.com/ABP-001")
            .unwrap();
    ...
    #[test]
    fn test_extract_info_value() {
        let scraper = JavbusScraper::new();
        let html_content = r#"
        <div class="info">
            <p><span class="header">Studio:</span> ABC Productions</p>
        </div>
        "#;
        let html = Html::parse_document(html_content);
        let value = scraper.extract_info_value(&html, "Studio:");
        assert_eq!(value, "ABC Productions");
    }
}
